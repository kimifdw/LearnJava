...
11. CAS(Compare and Swap): CAS操作需要输入两个值，一个旧值和一个新值，在操作期间先比较下旧值有没有发生变化，如果没有发生变化，才交换成新值，发生了变化则不交换。利用处理器提供的CMPXCHG指令实现的，而处理器执行CMPXCHG指令是一个原子性操作。
...
41. 缓存一致性。参考http://www.infoq.com/cn/articles/cache-coherency-primer。
    1. 核心思想。当某个CPU在写数据时，如果发现操作的变量是共享变量，则会通知其他CPU告知该变量的缓存行是无效的。

42. 原子性。即一个操作或多个操作，要么全部执行并且执行的过程不会被任何因素打断，要么就能不执行。

43. 可见性。当多个线程访问同一个变量时，一个线程修改了这个变量的值，其他线程能够立即看到修改的值。

44. 有序性。即程序执行的顺序按照代码的先后顺序执行。

45. happens-before原则。【先行发生原则】
    1. 程序次序原则。【单线程】
        1. 定义。一个线程内按照代码顺序，书写在前面的操作先行发生于书写在后面的操作，最终执行的结果与执行的顺序无关。
