 ...
 */
public class KafkaSimpleConsumerAPI {
    // 最大重试次数
    ...
    public void run(long maxReads,
        ...
        String topic = topicPartitionInfo.getTopic();
        int partitionID = topicPartitionInfo.getPartitionID();
    ...
    public PartitionMetadata findLeader(
        ...
        for (KafkaBrokerInfo broker : brokers) {
            ...
            try {
                // 1. 创建简单的消费者连接对象
                consumer = new SimpleConsumer(broker.getBrokerHost(), broker.getBrokerPort(), 100000, 64 * 1024, "leaderLookUp");
                ...
                System.out.println("Error communicating with Broker [" + broker.getBrokerHost() + "] to find Leader for [" + topic + ", " + partitionID + "] Reason:" + e);
            } finally {
                if (consumer != null) {
                    try {
                        consumer.close();
    ...
    public long getOffsetOfTopicAndPartition(SimpleConsumer consumer, String groupId, String clientName, String topic, int partitionID) {
        ...
        if (returnOffsetMetadata != null && !returnOffsetMetadata.isEmpty()) {
            ...
            if (offset.canEqual(ErrorMapping.NoError())) {
                // 没有异常，表示是正常的，获取偏移量
    ...
    public static List<Integer> fetchTopicPartitionIDs(List<KafkaBrokerInfo> brokers, String topic, int soTimeout, int bufferSize, String clientId) {
        ...
        for (KafkaBrokerInfo broker : brokers) {
            ...
            try {
                // 构建简单消费者连接对象
                consumer = new SimpleConsumer(broker.getBrokerHost(), broker.getBrokerPort(), soTimeout, bufferSize, clientId);
