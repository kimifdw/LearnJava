...
24. 竞态条件：当两个线程竞争**同一资源**时，并且其中的一个或者多个线程对这个资源进行了写操作，就称为存在竞态条件。多个线程同时读同一个资源**不会**产生竞态条件。
25. 临界区：导致竞态条件发生的代码区。
26. 互斥：这个资源只有我用，你不许用；原子：我一口气把活干完，不间断。
27. 线程安全的代码：允许被多个线程同时执行的代码
28. 局部变量：存储在线程自己的栈中，永远不会被多个线程共享，所以基础类型的局部变量是线程安全的。
29. 线程控制逃逸规则。如果一个资源的创建、使用、销毁都在同一个线程内完成，且永远不会脱离该线程的控制，则该资源的使用就是线程安全的。
30. 监视器对象：在同步构造器中用括号括起来的对象，一次只有一个线程能够在同步于同一个监视器对象的java方法内执行。
## 并发模型
...
4. 可见性：一个线程修改了这个变量的值，会立马写回主存储器中，这样就能被另一个线程看到这个变量的值得变化（**多线程是并发不是并行**）。确保编译器不会对代码作寄存器优化
## synchronized
